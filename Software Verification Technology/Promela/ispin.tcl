...
set xversion "iSpin Version 1.0.3 -- 4 January 2011"
...
proc swarm_gen { t } {
	...
	if {[string first "C:" $Fname] >= 0 || [string first "/" $Fname] == 0} {
		catch { set fd [open "|sh $Fname.swarm" r] } errmsg
	} else {
		catch { set fd [open "|sh ./$Fname.swarm" r] } errmsg
	}
	if {$fd == -1} {
...
proc cleanup {} {
	...
	catch { eval exec $RM pan.t pan.m pan.h pan.c pan.b pan.pre run.tmp pan.exe pan }
}
